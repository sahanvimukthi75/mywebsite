...
const Homea = () => {
  ...
  useEffect(() => {
    if (id) {
      axios.get(`http://54.89.169.194:5000/posts/${id}`)
        .then(result => {
          const post = result.data;
          setFormData({
            location: post.location,
            description: post.description,
            image: null, // Reset to null to allow the user to upload a new file if they wish
  ...
  const handleSubmit = (e) => {
    ...
    if (id) {
      axios.put(`http://54.89.169.194:5000/update/${id}`, data)
        .then(res => {
      ...
      axios.post('http://54.89.169.194:5000/addpost', data)
        .then(res => {
